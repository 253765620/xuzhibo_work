...
class ZdmonLoginTable(Base):
    __tablename__ = ZDMON_TABLE_DICT['login']
    id = Column(Integer,primary_key=True,autoincrement=True)
    time_ter = Column(Integer,primary_key=True)
    time_pi = Column(Integer,primary_key=True)
    time_interval = Column(Integer)
    client_msg_vin = Column(String(40))
    login_id = Column(String(4))
    ICCID = Column(String(34))
    num_subsys = Column(String(2))
    len_coding = Column(String(2))
