...
class SplitBase:
    ...
    def __init__(self, val):
        ...
        self.flag = False
        if val:
            sign_len = str(val[5]+1)
            content_len_start = str(10+val[5])
            #content_len_stop = str(14+val[5])
            self.split_list = [
                'msg_header/2', 'main_com/1', 'msg_len/2', 'sign/'+sign_len,
                'msg_id/4','content/'+content_len_start+'~-7','check_code/6',
                'terminator/1',
            ]
            mydict = self.build_dict(val)
        '''
        if val:
            if self.crc_check:
    ...
    def build_dict(self, val):
        ...
        for index in range(split_list_length):
            ...
            if isinstance(field_range, list):
                ...
                split_range = split_range + len(field_value)  # The length of the split
            else:
