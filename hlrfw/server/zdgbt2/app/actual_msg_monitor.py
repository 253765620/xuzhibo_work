...
''' 
def get_int_data(val):
    val_len = len(val)
    ret_val = 0
    for item in val:
        val_len = val_len - 1
        ret_val = ret_val + item*256**val_len
    return ret_val

def get_2_data(val, num2, deviant=0):
    ret_list = []
    location = 0
    data_len = len(val)/num2
    for i in range(data_len):
        mylist = []
        for i in range(num2):
            mylist.append(val[location])
            location = location + 1
        ret_list.append(get_int_data(mylist)-deviant)
    return ret_list
'''
...
def chargeable_sys(msg_dict):
    ...
    if flag_6 and flag_8:
        ...
        #print max_unit_cell_vol_8,max_battery_vol_6,max_unit_cell_vol_id_8,max_vol_battery_id_6
        #print min_unit_cell_vol_8,min_battery_vol_6,min_unit_cell_vol_id_8,min_vol_battery_id_6
        #print max_temperature_9,max_temperature_6,max_temperature_id_9,max_temperature_probe_id_6    
        #print min_temperature_9,min_temperature_6,min_temperature_id_9,min_temperature_probe_id_6
