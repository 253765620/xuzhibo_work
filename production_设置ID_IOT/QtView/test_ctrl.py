...
class TestControl(object):
    ...
    def new_key_status(self,key_num):
        ...
        for i in range(20):
            try:
                a = self.serial.serial_send('IOT-C1+bg.p.but.sta=?', 'IOT',baudrate = self.baudrates)
            except:
                return -1
            if a == keys[key_num] and TestControl.led_test_flag==True:
                return 0
            sleep(0.4)
        return -1
    ...
    def new_pwoer_test(self):
        ...
        sleep(3)
        try:
            power_supply = self.serial.serial_send('IOT-S1+ENERGY=?', 'IOT', baudrates = self.baudrates)
    ...
    def new_relay_test(self):
        ...
        for i in range(10):
            if TestControl.led_test_flag:
                ...
                return 0
            self.serial.serial_send('IOT-V1+bg.p.relay.sw=1', 'IOT',baudrates = self.baudrates)
            sleep(0.5)
            self.serial.serial_send('IOT-V1+bg.p.relay.sw=2', 'IOT',baudrates = self.baudrates)
    ...
    def new_gpio_test(self):
        ...
        TestControl.led_test_flag = False
        try:
            gpio2_value_1 = self.serial.serial_send('IOT+V1+bg.p.lev4.sw=?','IOT',baudrate=self.baudrates)
            if gpio2_value_1 != '1':
                self.infor.emit('GPIO测试失败')
                return -1
            self.serial.serial_send('IOT+V1+bg.p.lev1.sw=0', 'IOT', baudrate=self.baudrates) #拉低3.3V电压
            self.serial.serial_send('IOT+V1+bg.p.lev2.sw=0', 'IOT', baudrate=self.baudrates) #拉低12V电压
            self.serial.serial_send('IOT+V1+bg.p.lev3.sw=0', 'IOT', baudrate=self.baudrates) #拉低5V电压
            sleep(2)
            gpio2_value_1 = self.serial.serial_send('IOT+V1+bg.p.lev4.sw=?', 'IOT', baudrate=self.baudrates)
            if gpio2_value_1 != '0':
                self.infor.emit('GPIO测试失败')
                ...
                self.serial.serial_send('IOT+V1+bg.p.lev1.sw=1', 'IOT', baudrate=self.baudrates)  # 拉高3.3V电压
                self.serial.serial_send('IOT+V1+bg.p.lev2.sw=1', 'IOT', baudrate=self.baudrates)  # 拉高12V电压
                self.serial.serial_send('IOT+V1+bg.p.lev3.sw=1', 'IOT', baudrate=self.baudrates)  # 拉高5V电压
                for i in range(20):
                    if TestControl.led_test_flag:
    ...
    def new_key_and_led_test(self):
        ...
        for item in [1, 2, 3, 4]:
            ...
            if key_status==0:
                self.infor.emit('按键{}LED{}测试通过'.format(item,item))
                #self.add_test_record(0, '按键{}'.format(item))
            else:
                self.infor.emit('按键{}LED{}测试失败'.format(item,item))
